...
export default function FormUpdateRental({ products, users, clients,origin,rental }: FormRentalProps) {
    ...
    const createDresss = useMutation(async (rentalUpdate: CreateRentalFormData) => {
        try { 
    ...
    async function onInsertNewRental(form: CreateRentalFormData) {
        ...
        setErrors('');
        setSuccess('');
        console.log(form.expected_delivery_date);
        ...
        <>
            <div className={`${styles.container}`}>
                <form onSubmit={handleSubmit(onInsertNewRental)} method="post" >
                    ...
                    <div className={`${styles.containerInputs}`}>
                        <div>
                            <SelectTrait text="Produto" name="dress_id" options={products} register={register} control={control} currentValue={rental?.dress_id} />
                            <LabelValidate message={errors.dress_id?.message} />

                        </div>
                        <div>
                            <SelectTrait text="Vendedor" name="user_id" options={users} register={register} control={control} currentValue={rental?.user_id}/>
                            <LabelValidate message={errors.user_id?.message} />

                        </div>
                        <div>
                            <SelectTrait text="Clientes" name="client_id" options={clients} register={register} control={control} currentValue={rental?.client_id}/>
