...
export default function FormUpdateRental({ products, users, clients,rental }: FormRentalProps) {
    ...
    const createRentals = useMutation(async (rentalUpdate: CreateRentalFormData) => {
        try { 
    ...
    async function onInsertNewRental(form: CreateRentalFormData) {
        ...
        await createRentals.mutateAsync(rental);
        setLoading(false);
        ...
        <>
            <div className={`${styles.container}`}>
                <form onSubmit={handleSubmit(onInsertNewRental)} method="post" >
                    ...
                    <div className={`${styles.containerInputs}`}>
                        <div>
                            <SelectTrait text="Produto" name="product_id" options={products} register={register} control={control} currentValue={rental?.product_id} />
